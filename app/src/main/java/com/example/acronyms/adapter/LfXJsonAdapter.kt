...
public class LfXJsonAdapter(
    moshi: Moshi
) : JsonAdapter<LfX>() {
    private val options: JsonReader.Options = JsonReader.Options.of("freq", "lf", "since", "vars")
    ...
    private val listOfVarXAdapter: JsonAdapter<List<VarX>> =
        moshi.adapter(
            Types.newParameterizedType(List::class.java, VarX::class.java),
            emptySet(),
            "vars"
        )

    public override fun toString(): String = buildString(25) {
        append("GeneratedJsonAdapter(").append("LfX").append(')')
    }

    public override fun fromJson(reader: JsonReader): LfX {
        var freq: Int? = null
        var lf: String? = null
        var since: Int? = null
        ...
        while (reader.hasNext()) {
            when (reader.selectName(options)) {
                ...
                3 -> vars = listOfVarXAdapter.fromJson(reader) ?: throw Util.unexpectedNull(
                    "vars",
                    "vars",
                    reader
                )
        ...
        return LfX(
            ...
            vars = vars ?: throw Util.missingProperty("vars", "vars", reader)
        )
    }

    public override fun toJson(writer: JsonWriter, value_: LfX?) {
        if (value_ == null) {
        ...
        writer.name("vars")
        listOfVarXAdapter.toJson(writer, value_.vars)
        writer.endObject()
    }
}
